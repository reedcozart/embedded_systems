...
void PortE_Init(void){
  ...
  GPIO_PORTE_AFSEL_R &= ~0x1;  //     disable alt funct onPE0
  GPIO_PORTE_DEN_R |= 0x01;     //     enable digital I/O on PE0   
  //GPIO_PORTE_PCTL_R &= ~0x000F0000; // configure PE0 as GPIO
	GPIO_PORTE_PCTL_R = 0x00000000; // configure  PE0 as GPIO
  ...
  GPIO_PORTE_ICR_R = 0x01;      // (e) clear flag0
	GPIO_PORTE_IM_R |= 0x01;      // (f) arm interrupt on PE0  *** No IME bit as mentioned in Book ***
  NVIC_PRI7_R = (NVIC_PRI1_R&0xFFFFFF00)|0x00000040; // (g) priority 2
  NVIC_EN0_R |= 0x00000011;      // (h) enable interrupt 4 in NVIC
}
